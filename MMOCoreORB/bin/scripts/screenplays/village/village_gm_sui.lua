...
function VillageGmSui.fixCouncilType(pPlayer, targetID)
	...
	if (luaCouncil ~= JediTrials.COUNCIL_LIGHT and luaCouncil ~= JediTrials.COUNCIL_DARK and frsCouncil ~= JediTrials.COUNCIL_LIGHT and frsCouncil ~= JediTrials.COUNCIL_DARK) then
		if (CreatureObject(pTarget):getFaction() == FACTIONREBEL) then
			PlayerObject(pGhost):setFrsCouncil(1)
			JediTrials:setJediCouncil(pTarget, 1)
			frsCouncil = 1
			luaCouncil = 1
			PlayerObject(pGhost):setFrsRank(0)
		end
		if (CreatureObject(pTarget):getFaction() == FACTIONIMPERIAL) then
			PlayerObject(pGhost):setFrsCouncil(2)
			JediTrials:setJediCouncil(pTarget, 2)
			PlayerObject(pGhost):setFrsRank(0)
			frsCouncil = 2
			luaCouncil = 2
		end
		if (CreatureObject(pTarget):getFaction() ~= FACTIONIMPERIAL and CreatureObject(pTarget):getFaction() ~= FACTIONREBEL) then
			CreatureObject(pPlayer):sendSystemMessage("Council type cannot be set for a factionless knight.")
		return
		end
	end
