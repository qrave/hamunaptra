...
int CombatManager::getAttackerAccuracyModifier(TangibleObject* attacker, CreatureObject* defender, WeaponObject* weapon) {
	...
	if (weapon->getAttackType() == SharedWeaponObjectTemplate::MELEEATTACK){
		...
		//error("PreFRS accuracy bonus:" + String::valueOf(attackerAccuracy));
		attackerAccuracy *= frsBonus; //bonus Accuracy for FRS manipulation
		//error("PostFRS accuracy bonus:" + String::valueOf(attackerAccuracy));
	}
...
float CombatManager::getDefenderToughnessModifier(CreatureObject* defender, int attackType, int damType, float damage) {
	...
	if (damType == SharedWeaponObjectTemplate::LIGHTSABER && jediToughness > 0)
		damage *= 1.f - ((jediToughness/(8.5*frsBonus)) / 100.f); // portion of Jedi toughness allowed to act as Lightsaber Toughness
...
int CombatManager::getArmorReduction(TangibleObject* attacker, WeaponObject* weapon, CreatureObject* defender, float damage, int hitLocation, const CreatureAttackData& data) {
	...
	if (!data.isForceAttack()) {
		...
		forceArmor *= defender->getFrsMod("manipulation");
		if (forceArmor > 0) {
			...
			if (attacker->isPlayerCreature())
				defender->notifyObservers(ObserverEventType::FORCEBUFFHIT, attacker, dmgAbsorbed * 1.55); //50% penalty for PVP force cost
			else
			...
			sendMitigationCombatSpam(defender, NULL, (int)dmgAbsorbed, FORCEARMOR);
				...
				// Force Shield
				float forceShield = defender->getSkillMod("force_shield");
				forceShield *= defender->getFrsMod("control");
				if (defender->hasBuff(BuffCRC::JEDI_FORCEBREACH))
				...
				if ( absorbSkill > 0 && defender->isPlayerCreature() && !defender->hasBuff(BuffCRC::JEDI_FORCEBREACH) && defender->getSkillMod("avoid_incapacitation") < 1) { //No absorb while using AI
					...
					if (attacker->isPlayerCreature()){
						damageToAbsorb *= 0.15f; // nerf absorbed force vs PC by 98%
	...
	if (psg != NULL && !psg->isVulnerable(damageType) && !hasForceArmor) {
		...
		if (defender->hasBuff(BuffCRC::JEDI_FORCEBREACH))
				armorReduction *= .25; //Force breach bypasses 75% of your PSG
...
float CombatManager::calculateDamage(CreatureObject* attacker, WeaponObject* weapon, TangibleObject* defender, const CreatureAttackData& data) {
	float damage = 0;
	int diff = 0;
	CreatureObject* creoDefender = nullptr;
	...
	uint32 intimCRC = Long::hashCode(CreatureState::INTIMIDATED);

	if (defender->isPlayerCreature())
		creoDefender = defender->asCreatureObject();

	if (creoDefender != nullptr){
		if (creoDefender->isIntimidated()){
			damage *= attacker->getFrsMod("control",2); //Up to a 12% damage bypass for intimidate based on control score
		}
		if (attacker->isIntimidated()){
			damage /= creoDefender->getFrsMod("power"); //Up to a 25% damage reduction for intimidate based on power score
			PlayerObject* attackerGhost = attacker->getPlayerObject();
			if (attackerGhost != NULL){
				if ((attacker->getHAM(CreatureAttribute::HEALTH) > creoDefender->getHAM(CreatureAttribute::HEALTH) * .65) && attackerGhost->hasBhTef()){ //intimidate is less effective against hunters.
					if (weapon->getAttackType() == SharedWeaponObjectTemplate::MELEEATTACK)
						damage *= 1.35;
					else
						damage *= 1.15;
	...
	if (attacker->hasBuff(BuffCRC::SKILL_BUFF_HUNTER) && creoDefender->getHuntedLevel(creoDefender) > 0)
		damage *= .85; //Mandalorian hunter buff.
...
float CombatManager::calculateDamage(CreatureObject* attacker, WeaponObject* weapon, CreatureObject* defender, const CreatureAttackData& data) {
	...
	if (defender->isKnockedDown() && defender->isPlayerCreature() && !attacker->isPlayerCreature())
        ...
        if (defender->hasBuff(BuffCRC::JEDI_FORTIFY) && !attacker->isIntimidated() && attacker->checkCooldownRecovery("fortify_intimidate"))
        {
                ...
                attacker->updateCooldownTimer("fortify_intimidate", 5000 + System::random(2000));
        }
	...
	if (weapon->isHeavyWeapon()){
		...
		if (attacker->isPlayerCreature()){ //player is using force powers, apply cloak of pain buff
			...
			if (playerObject != NULL && attacker->getSkillMod("avoid_incapacitation") < 1){ //No Force return while using Avoid incap
			    float forceRefreshSkill = 2.5 * (defender->getSkillMod("cloakofpain") * .6); //this skill mod scales with investment in force power, convenient to use
				int maxForce = playerObject->getForcePowerMax();
				int currentForce = playerObject->getForcePower();
				float siphon =  (damage * .01) * (1 + (forceRefreshSkill/100)); //This will be the amount of force returned (DMG x .0162) at Mpowers
																					//a 10k hit for instance would give 162 force back at Mpowrs

				if (wielded != NULL){ //Reduced force power returned based on saber Force cost
						float fc = wielded->getForceCost();
				...
				if (playerObject != NULL && wielded != NULL && wielded->isJediWeapon()){
						...
						if (saberInv != NULL){
							...
							if (i < 1)
								i = 1;
							damageAverage /= i;
							damage *= (.65f + (damageAverage/ 100.f)); //up to a 35% penalty for poor crystal averages or no saber

				}
			}

			int cloakOfPain;
			cloakOfPain = defender->getSkillMod("cloakofpain");
			float checkForceArmor = attacker->getSkillMod("force_armor");
			float checkForceShield = attacker->getSkillMod("force_shield");
			...
			if (defender->isPlayerCreature() && cloakOfPain > 0){ //defender has cloak of pain
				float dmgAbsorbed = 0, diff=0;
					diff = (damage * (1.f - (cloakOfPain / 100.f)));
					dmgAbsorbed = damage - diff;
					sendMitigationCombatSpam(defender, NULL, (int)dmgAbsorbed, FORCESHIELD);
					damage -= (int)dmgAbsorbed;
			}
...
int CombatManager::getHitChance(TangibleObject* attacker, CreatureObject* targetCreature, WeaponObject* weapon, int damage, int accuracyBonus) {
	...
	if (damage > 0) {
		...
		if (def == "saber_block") {
			...
			if (attacker->isPlayerCreature() && saberBlock > (50 * targetCreature->getFrsMod("manipulation",2)))
				saberBlock = 55 * targetCreature->getFrsMod("manipulation",2); //saber block maximum in pvp is 55%, further modified up to 75% with FRS

			saberBlock -= targetCreature->getHuntedLevel(targetCreature) * 5; //mark of mandalore debuff

			if (saberBlock > 85) //Saber block maximum in PVE remains 85%, with 85% being the hard card.
				saberBlock = 85;
...
bool CombatManager::applySpecialAttackCost(CreatureObject* attacker, WeaponObject* weapon, const CreatureAttackData& data) {
	...
	if (attackerGhost != NULL && weaponForceCost > 0)
		if ((attackerGhost->hasPvpTef() || attackerGhost->hasBhTef() || attackerGhost->getDuelListSize() != 0) && weaponForceCost < 5)
			weaponForceCost = 5;
...
void CombatManager::applyStates(CreatureObject* creature, CreatureObject* targetCreature, const CreatureAttackData& data) {
	...
	for (int i = 0; i < stateEffects->size(); i++) {
		...
		if (defenderJediWeapon) //fighting another jedi, apply FRS bonus chance to states
			accuracyMod *= targetCreature->getFrsMod("power",2);
		...
		if (!failed) {
			...
			float frsBonus = targetCreature->getFrsMod("manipulation",1.5);
				targetDefense = 0.f;
				...
				for (int j = 0; j < jediMods.size(); j++){
					...
					jsrMod += targetCreature->getSkillMod(jediMods.get(j));
				}
