  ...
  int curExperience = ghost->getExperience("force_rank_xp");
    ...
    playerManager->awardExperience(player, "force_rank_xp", amount);
		...
		if (councilType == COUNCIL_LIGHT)
			...
			error("FRS: Player [" + player->getFirstName() + "] Rank recognized as [" + rank + "]");
		}
		if (rankingData == NULL){
			error("FRS: NO ranking data found, aborting");
			return;
		}
	...
	//player->sendSystemMessage(param);
		...
		if (reqXp <= curExperience){  //They have gained enough experience to autopromote
			error("Player: " + player->getFirstName() + " has enough xp to rank, promoting to rank: [" + String::valueOf(rank+1) + "]" );
