...
void PlayerManagerImplementation::enhanceCharacter(CreatureObject* player) {
	...
	player->sendSystemMessage("4000 credits has been deducted for buffs.");
	}
...
int PlayerManagerImplementation::calculateAttackers(CreatureObject* victim, CreatureObject* killer, ThreatMap* threatMap){
	...
	for (int i = 0; i < threatMap->size(); ++i) { //we should have been passed the victim's threatmap here as initial arguments for the function
		...
		error("calculateAttackers: Info on participant: [" + attacker->getFirstName() + "]");

		if (attacker->isGrouped()){
			if (attacker->getGroupID() == groupId){
				error("calculateAttackers: Participant is in killer or defender's group, not counting as part of threatmap");
				continue; //dont double count group members
			}

			if (attacker->getZone() != victim->getZone()){
			error("calculateAttackers: " + attacker->getFirstName() + " is not in the same zone as the victim, skipping");
			continue;
			}
		        if (CombatManager::instance()->areInDuel(attacker, victim)){
		                error("calculateAttackers: Member and victim are in a duel, no FRS granted");
				continue;
		        }
		}
		    //    if (CombatManager::instance()->areInDuel(attacker, victim)){
		...
		if (!allowSameAccountPvpRatingCredit && defenderGhost->getAccountID() == attackerGhost->getAccountID()){
			error("calculateAttackers ThreatMap: Participant is another character on the same account, disallowing FRS Credit");
			continue;
		}

		if (isSameFaction(victim, attacker)){
			error("calculateAttackers ThreatMap: Participant is same faction, skipping");
			continue; //Cant be same faction
		}

		if (pos.distanceTo(attacker->getWorldPosition()) > 256){
			error("calculateAttackers ThreatMap: Participant is out of range: [" + String::valueOf(pos.distanceTo(victim->getWorldPosition())) + "]");
			continue;
		}
		error("calculateAttackers ThreatMap: Participant added to threatmap total.");
		count++;
		}
		...
		if (group == NULL){
			error("calculateAttackers Killer/Defender is ungrouped, bypassing group check");
			return count;
		}
		...
		for (int i = 0; i < group->getGroupSize(); i++) {
			...
			if (member->getFactionStatus() != FactionStatus::OVERT && !memberGhost->hasBhTef()){
				error("calculateAttackers GroupCheck: Participant is not overt or a BH with an active mission, not counting.");
			...
			error("calculateAttackers: " + member->getFirstName() + " is not in the same zone as the victim, skipping");
			continue;
			}
			...
			if (isSameFaction(victim, member)){
				error("calculateAttackers GroupCheck: Participant is same faction, skipping");
				continue; //Cant be same faction
			}

			if (pos.distanceTo(member->getWorldPosition()) > 256){
				error("calculateAttackers GroupCheck: Participant is out of range for credit - Distance: [" + String::valueOf(pos.distanceTo(member->getWorldPosition()))  + "]");
...
void PlayerManagerImplementation::awardPvpProgress(TangibleObject* killer, CreatureObject* destructedObject, bool performedDeathBlow = false) {
	...
	if (!destructedObject->isRebel() && !destructedObject->isImperial() && !killedGhost->hasBhTef()){
		error("PVP Victim is not factionally aligned or a BH, no FRS Granted");
		return; //The victim is not factionally aligned and is therefor granted no PVP progress
	}

	//Lets see how many were involved.
	error("FRS: Involved Attackers for the death of: " + destructedObject->getFirstName());
	int countAttackers = calculateAttackers(destructedObject,killerCreature,attackerThreatMap);
	error("FRS: Involved Defenders for the death of: " + destructedObject->getFirstName());
	int countDefenders = calculateAttackers(killerCreature,destructedObject,victimThreatMap);

	error("New FRS Kill [" + String::valueOf(countAttackers) + "] Attackers, [" + String::valueOf(countDefenders) + "] Defenders");
  ...
  error("Award: +Bounty Hunter Bonus = " + String::valueOf(award));
	...
	error("Award: +Rank Bonus" + String::valueOf(award));

	if (countDefenders < 1 || countAttackers < 1){
	error ("Defender or attacker counter is below 1, this should not be possible!");
	return; //Wtf?
	}

	float sizeRatio = Math::max(1,countAttackers/countDefenders);
	error("Award: ratio of the difference in group size calculated to: " + String::valueOf(sizeRatio));
	...
	error("Award: modfiiedAward = " + String::valueOf(modifiedAward));
	...
	if (group != NULL){//apply final penalty of up to 50% based on group size
		error("Killer is grouped, adjusting down for group members, maximum penalty of 1/2");
		float gSize = Math::min(10,group->getGroupSize());// get the group size, cap it at 10

		error("Award: groupSize value = " + String::valueOf(gSize));

		float groupPenalty = (gSize * 5)/100;
		error("Award: Group Size Penalty = " + String::valueOf(groupPenalty));
		finalAward = finalAward * groupPenalty;
		error("Award: After Group Size Penalty = " + String::valueOf(finalAward));
		if (finalAward < (modifiedAward/2)){
		error("Final payout calculated to less than half original value, increasing to half payout) New Value = " + String::valueOf(finalAward));
	...
	if (sizeRatio < 1){ //Killer won against overwhelming odds, they are granted a bonus of 25% for each additional attacker
		error("Victim Won against a larger attacking force, bonus applied.");
	...
	if (!allowSameAccountPvpRatingCredit && ghost->getAccountID() == killedGhost->getAccountID()){
		error("No killer award allowed, same account");
		awardAllowed = false;
	}

	if (killerCreature->getFactionStatus() != FactionStatus::OVERT){
		error("Killer is not Overt, no FRS granted");
		awardAllowed = false;
	}

	if (CombatManager::instance()->areInDuel(killerCreature, destructedObject)){
                error("Killer and victim are in a duel, no FRS granted");
                awardAllowed = false;
	}
	...
	if ((killerCreature->hasSkill("force_rank_light_novice") || killerCreature->hasSkill("force_rank_dark_novice"))){
		...
		if (killedGhost->hasBhTef() && !destructedObject->hasSkill("combat_bountyhunter_investigation_03")){
			error("Victim has recently, or is actively beening hunted by a BH, no FRS is awarded");
			...
			error("Award: Paying FRS to Killer, final Award:" + String::valueOf(finalAward));
	...
	Vector3 pos(destructedObject->getWorldPositionX(), destructedObject->getWorldPositionY(), 0);
		...
		if (group != NULL) {
			error("Award: Killer is grouped, paying out FRS to all eligible");
			for (int i = 0; i < group->getGroupSize(); i++) {
				CreatureObject* member = group->getGroupMember(i);
				PlayerObject* memberGhost = member->getPlayerObject();
				...
				error(member->getFirstName() + " is not in the same zone as the victim, skipping");
				continue;
				}

				error("Award: Start Check for Participant [" + member->getFirstName() +  "]");

				if (member == NULL || memberGhost == NULL || member == killerCreature || !member->isPlayerCreature() || member == destructedObject){
					error("Participant is Killer or Victim, skipping: " + member->getFirstName());
				continue; //dont count the killer twice, dont count pets, dont count the victim if they some how managed to hurt themselves
			        ...
			        if (CombatManager::instance()->areInDuel(member, destructedObject)){
			                error("Member and victim are in a duel, no FRS granted, skipping: " + member->getFirstName());
					continue;
			        }

				if (member->getFactionStatus() != FactionStatus::OVERT){
					error("Participant is not Overt, skipping: " + member->getFirstName());
				continue; //we're only interested in overts
 				}

				if (isSameFaction(destructedObject, member)){
					error("Participant is same faction, skipping, " + member->getFirstName());
 					continue; //Cant be same faction
				}

				if (pos.distanceTo(member->getWorldPosition()) > 256){
					error(member->getFirstName() + "is Out of range of " + destructedObject->getFirstName() + ", Distance: [" + String::valueOf(pos.distanceTo(member->getWorldPosition())) + "] ms");
				...
				if (!killedGhost->hasBhTef() || destructedObject->hasSkill("combat_bountyhunter_investigation_03")){
					...
					if (member->hasSkill("force_rank_light_novice") || member->hasSkill("force_rank_dark_novice")){
						...
						error("Award: Granting FRS to Participant [" + String::valueOf(finalAward) + "]");
	...
	error("Award: deducion XP calculated to .65 of modifiedAward = " + String::valueOf(deductionXp));

	if (sizeRatio >= 4){
	deductionXp = 0;
        error("Award: Overhwelming odds protection activated, no FRS XP Lost");
	}
...
int PlayerManagerImplementation::getRank(CreatureObject *player){
	...
	if (player->hasSkill("force_rank_light_novice") || player->hasSkill("force_rank_dark_novice")){ //They are part of FRS, pull their rank here and return it
		...
		if (ghost == NULL){
			error("Ghost is invalid in FRS rank lookup");
			return 0;
		}
