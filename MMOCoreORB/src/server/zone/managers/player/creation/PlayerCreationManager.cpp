...
bool PlayerCreationManager::createCharacter(ClientCreateCharacterCallback* callback) const {
	...
	if (client->getCharacterCount(zoneServer.get()->getGalaxyID()) >= 7) {
		ErrorMessage* errMsg = new ErrorMessage("Create Error", "Trying to create an 8th character? No luck for you sunshine.", 0x0);
				...
				if (accountPermissionLevel < 9) {
					try {
						...
						if (res != NULL && res->next()) {
							...
							if (timeVal.miliDifference() < 36000) {
	...
	box->setPromptText("With the warmest of welcomes, hello! Join Infinity Chat channel to speak with the other Infinians and explore Naboo, Tatooine and Lok for the main Player City hubs. There are numerous player-created buffing centers and solo groups, just ask in Infinity chat! If you need anything else come and join our Discord server (link on our website swginfinity.com). Make sure to join our Discord (link on our website swginfinity.com)");
